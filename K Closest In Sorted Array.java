...
public class Solution {
  public int[] kClosest(int[] array, int target, int k) {
    ...
    int left = noMoreThan(array, target);
    int right = left + 1;
    
    int[] result = new int[k];
    for (int i = 0; i < k; i++){
      //right out of bound; both in the bound, left close to the target, return left
      if (right >= array.length || left >= 0 && target - array[left] <= array[right]-target){
        result[i] = array[left--];
      } else {  //if left < 0, right == 0, always right ++
  ...
  private int noMoreThan(int[] array, int target){
    int left = 0;
    int right = array.length - 1;
    // find the number no more than target return index, if not find return -1
    while (left + 1 < right){
